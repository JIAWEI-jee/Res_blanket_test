...
{
 float temp =0,res = 0;
 u16 ch0_v = 0,ch1_v = 0,ch2_v = 0,ch3_v = 0;
 Voltg_calc ( &ch0_v,&ch1_v,&ch2_v,&ch3_v );
 temp = (  ( ( float ) ch0_v /1000 ) - ( ( float ) ch1_v/1000 ) ) / ( ( ( float ) ch1_v/1000 ) /510 );
 KEY_printf ( "temper_res = %f \r\n",temp ); //pjw set
 *temper_res = (u16)temp;
 KEY_printf ( "temper_res = %d \r\n",* temper_res ); //pjw set
 temp = (  ( ( float ) ch2_v /1000 ) - ( ( float ) ch3_v/1000 ) ) / ( ( ( float ) ch3_v/1000 ) /510 );
 KEY_printf ( "temper_res = %f \r\n",temp ); //pjw set
 *heat_res = (u16)temp;
 KEY_printf ( "temper_res = %d \r\n",* temper_res ); //pjw set
}
